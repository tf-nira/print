...
public interface CardDetailRepository extends JpaRepository<CardDetail, String> {
	...
	@Query(value ="SELECT * FROM print.card_detail c WHERE c.is_ready_to_push=true AND c.is_pushed=false AND c.is_failed IS NOT TRUE AND is_processing IS NOT TRUE order by c.upd_dtimes FOR UPDATE SKIP LOCKED LIMIT :fetchSize", nativeQuery = true)
