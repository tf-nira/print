...
public interface NotificationStatusRepository extends JpaRepository<NotificationStatus, String> {
    ...
    @Query(value ="SELECT * FROM print.notification_status n WHERE n.notification_sent IS NOT TRUE AND n.is_processing IS NOT TRUE AND n.attributes IS NOT NULL order by n.cr_dtimes FOR UPDATE SKIP LOCKED LIMIT :fetchSize", nativeQuery = true)
